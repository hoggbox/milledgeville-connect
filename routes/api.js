...
// =============================================================================
// SCHEDULED NOTIFICATIONS — Admin push notification scheduler
// Pattern mirrors shoutout-thumb / business-post-thumb exactly:
//   base64 stored on the doc → served as real https:// bytes → passed to broadcastPush
// DO NOT change the shoutout/traffic-alert logic above — this is self-contained.
// =============================================================================

// ── Inline model (no separate file needed) ────────────────────────────────────
const ScheduledNotification = mongoose.models.ScheduledNotification ||
  mongoose.model('ScheduledNotification', new mongoose.Schema({
    title:       { type: String, required: true },
    body:        { type: String, required: true },
    image:       { type: String, default: null },   // base64 data URL, same as Shoutout.images[]
    business:    { type: mongoose.Schema.Types.ObjectId, ref: 'Business', default: null },
    targetType:  { type: String, default: 'home' }, // 'business' | 'business-post' | 'external' | 'app' | 'home'
    targetId:    { type: String, default: null },
    targetUrl:   { type: String, default: null },
    scheduledFor:{ type: Date,   default: null },
    repeat:      { type: String, default: 'once' }, // 'once' | 'custom'
    days:        { type: [String], default: [] },   // ['Mon','Wed','Fri'] for custom repeat
...
// ── GET /api/scheduled-notif-thumb/:id ───────────────────────────────────────
// Serves the stored base64 image as real HTTP bytes — exactly like shoutout-thumb.
// This is what gets passed to broadcastPush as imageUrl so FCM shows the image.
router.get('/scheduled-notif-thumb/:id', async (req, res) => {
  try {
    ...
    console.error('Scheduled notif thumb error:', err);
    res.status(500).send('Error');
  }
});
...
router.get('/admin/scheduled-notifications', authenticate, requireAdmin, async (req, res) => {
  try {
    const notifs = await ScheduledNotification
      .find({})
...
// Saves a new notification doc. Image stored as base64 directly on the doc,
// just like Shoutout.images[] — the thumb endpoint serves it at broadcast time.
router.post('/admin/scheduled-notifications', authenticate, requireAdmin, async (req, res) => {
  try {
    const { title, body, image, business, targetType, targetId, targetUrl,
            scheduledFor, repeat, days } = req.body;

    if (!title?.trim() || !body?.trim()) {
      return res.status(400).json({ message: 'title and body are required' });
    }

    // Validate image if provided — same rules as shoutout / business-post
    if (image) {
      if (!/^data:image\/(jpeg|png|webp);base64,/.test(image)) {
...
// Two modes:
//   { action: 'send-now' }  → immediately broadcast and mark sent
//   { ...fields }           → update the doc fields
router.patch('/admin/scheduled-notifications/:id', authenticate, requireAdmin, async (req, res) => {
  try {
    ...
    if (req.body.action === 'send-now') {
      // Build the thumb URL exactly like shoutout-thumb — a real https:// endpoint
      const imageUrl = notif.image
        ? `https://www.milledgevilleconnect.com/api/scheduled-notif-thumb/${notif._id}`
        : null;

      // Build deep-link data for the notification
      const data = {};
      if (notif.targetType === 'business' || notif.targetType === 'directory') {
        data.page = 'business'; data.id = notif.targetId || notif.business?.toString() || '';
      } else if (notif.targetType === 'business-post') {
        data.page = 'business-post'; data.id = notif.targetId || '';
      } else if (notif.targetType === 'external') {
        data.url = notif.targetUrl || '';
      } else if (notif.targetType === 'app') {
        data.page = notif.targetId || 'home';
      }

      // Fire and forget — same pattern as all other broadcastPush calls
      broadcastPush(notif.title, notif.body, data, { type: 'custom', imageUrl });
    ...
    // ── field update ─────────────────────────────────────────────────────────
    const { title, body, image, business, targetType, targetId, targetUrl,
            scheduledFor, repeat, days, status } = req.body;

    if (title !== undefined)       notif.title       = title.trim();
    if (body  !== undefined)       notif.body        = body.trim();
    if (business !== undefined)    notif.business    = business || null;
    if (targetType !== undefined)  notif.targetType  = targetType;
    if (targetId   !== undefined)  notif.targetId    = targetId  || null;
    if (targetUrl  !== undefined)  notif.targetUrl   = targetUrl || null;
    if (scheduledFor !== undefined) notif.scheduledFor = scheduledFor ? new Date(scheduledFor) : null;
    if (repeat !== undefined)      notif.repeat      = repeat;
    if (days   !== undefined)      notif.days        = days || [];
    if (status !== undefined)      notif.status      = status;

    // Image update — same validation as POST
    if (image !== undefined) {
      if (image === null || image === '') {
        notif.image = null;
      } else {
    ...
    console.error('Scheduled notif patch error:', err);
    res.status(500).json({ message: err.message });
  }
});
...
// ── Scheduler — runs every minute, fires pending notifications ────────────────
// Mirrors the same thumb-URL pattern: base64 on doc → https:// thumb → broadcastPush
async function runScheduledNotifications() {
  try {
    ...
    for (const notif of due) {
      try {
        // Build thumb URL — exactly like shoutout-thumb, not a data: URL
        const imageUrl = notif.image
          ? `https://www.milledgevilleconnect.com/api/scheduled-notif-thumb/${notif._id}`
          : null;
        ...
        await broadcastPush(notif.title, notif.body, data, { type: 'custom', imageUrl });
        console.log(`✅ [Scheduler] Sent: "${notif.title}" (${notif._id})`);

        if (notif.repeat === 'once') {
          // One-shot: mark sent permanently
          notif.status = 'sent';
          notif.sentAt = now;
        } else {
          // Recurring: advance scheduledFor to the next matching day
          const dayNames = ['Sun','Mon','Tue','Wed','Thu','Fri','Sat'];
          const validDays = (notif.days || []).filter(d => dayNames.includes(d));

          if (validDays.length > 0) {
            const next = new Date(notif.scheduledFor);
            next.setDate(next.getDate() + 1); // start looking from tomorrow
            for (let i = 0; i < 8; i++) {
              const dayName = dayNames[next.getDay()];
              if (validDays.includes(dayName)) break;
              next.setDate(next.getDate() + 1);
            }
            notif.scheduledFor = next;
            notif.sentAt = now;
            // status stays 'pending' so it fires again on next matching day
          } else {
            notif.status = 'sent';
            notif.sentAt = now;
          }
        ...
        console.error(`❌ [Scheduler] Failed to send notif ${notif._id}:`, sendErr);
        notif.status = 'failed';
        await notif.save();
      }
    }
  } catch (err) {
    console.error('[Scheduler] runScheduledNotifications error:', err);
  }
}
