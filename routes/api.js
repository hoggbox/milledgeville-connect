  ...
  try {
    ...
    if (receiverId.toString() !== req.userId.toString()) {
      await createNotification({
        ...
        actorAvatar: sender.profilePhoto || sender.avatar || null,
        type:        'message',
...
router.post('/shoutouts/:id/like', authenticate, async (req, res) => {
  try {
    ...
    if (wasNewLike && shoutout.authorId && shoutout.authorId.toString() !== req.userId) {
      const liker = await User.findById(req.userId).select('name profilePhoto avatar');
      if (liker) {
        createNotification({
          recipient:   shoutout.authorId,
          actor:       liker._id,
          actorName:   liker.name,
          actorAvatar: liker.profilePhoto || liker.avatar || null,
          type:        'like',
...
router.get('/shoutout-thumb/:shoutoutId', async (req, res) => {
  try {
    ...
    if (mimeType === 'image/jpg') mimeType = 'image/jpeg'; // normalize nonstandard browser type
    const buffer = Buffer.from(base64Data, 'base64');
